範例 H： FOREIGN KEY (ON DELETE SET NULL) 或 FOREIGN KEY (ON UPDATE SET NULL)，僅介紹 Column-Level 用法，Table-Level 請參考前面做法
USE TSQL2;
GO
...
( col1 int null constraint FK_tbl_Child_tbl_Parent REFERENCES tbl_Parent (col1) ON DELETE SET NULL,   --< 可在後面自行再加入 ON UPDATE SET NULL
  col2 varchar(10) not null )
go
-- END：Column-Level
...
-- 因為「子項」資料表的 FOREIGN KEY 已設定為 ON DELETE SET NULL

/* 若 FOREIGN KEY 有指定 ON UPDATE SET NULL
-- 更新「父項」資料列
update tbl_Parent set col1 = 10 where col1 = 1
-- (1 個資料列受到影響)：雖更新「父項」新增資料列，但「子項」相關資料列也受影響
-- 因為「子項」資料表的 FOREIGN KEY 已設定為 ON UPDATE SET NULL
*/
